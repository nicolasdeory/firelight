...
namespace LedDashboard.Modules.LeagueOfLegends.ChampionModules
{
    ...
    class NamiModule : ChampionModule
    {

        // Change to whatever champion you want to implement
        public const string CHAMPION_NAME = "Nami";
        ...
        public static NamiModule Create(int ledCount, GameState gameState, LightingMode preferredLightMode, AbilityCastPreference preferredCastMode = AbilityCastPreference.Normal)
        {
            return new NamiModule(ledCount, gameState, CHAMPION_NAME, preferredLightMode, preferredCastMode);
        }


        private NamiModule(int ledCount, GameState gameState, string championName, LightingMode preferredLightMode, AbilityCastPreference preferredCastMode)
                            : base(ledCount, championName, gameState, preferredLightMode, true)
        {
           ...
           /* animator.PreloadAnimation(ANIMATION_PATH + "Nami/q_cast.txt");
            animator.PreloadAnimation(ANIMATION_PATH + "Nami/w_cast.txt");
            animator.PreloadAnimation(ANIMATION_PATH + "Nami/e_cast.txt");
            animator.PreloadAnimation(ANIMATION_PATH + "Nami/r_cast.txt");*/
        ...
        {
                animator.RunAnimationOnce(ANIMATION_PATH + "Nami/q_cast.txt", timeScale: 1.5f);
        }

        private void OnCastW()
        {
                animator.RunAnimationOnce(ANIMATION_PATH + "Nami/w_cast.txt", timeScale: 0.5f);
        }

        private void OnCastE()
        {
                animator.RunAnimationOnce(ANIMATION_PATH + "Nami/e_cast.txt", timeScale: 0.1f);
        }

        private void OnCastR()
        {
            animator.RunAnimationOnce(ANIMATION_PATH + "Nami/r_cast.txt", timeScale: 0.3f);
        }
